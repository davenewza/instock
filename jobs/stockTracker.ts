...
export default StockTracker(async (ctx) => {
    ...
    for(const p of products) {
        for(const w of warehouses) {
            const available = await retrieveStockLevel(w.code, p.sku, ctx.secrets.TAKEALOT_API_KEY);

            let row = await models.stockLevels.where({
                date: new Date(),
                warehouseId: w.id,
                productId: p.id,
            }).findOne();
            ...
            if (row && row.available > available) {
                row = await models.stockLevels.update({id:row.id}, {available:available });
            }  else {
